...
The clip table is the primary navigation interface. It displays your input data as a sortable, paginated table with configurable columns. Click any row to load its audio and spectrogram.
...
- **Zoom and pan** — zoom into frequency and time regions using keyboard (`+`/`-`/`0`), mouse wheel, or click-and-drag. A view bounds bar shows the current time and frequency range in real time
