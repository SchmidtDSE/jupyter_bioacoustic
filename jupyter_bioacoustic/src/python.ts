...
export function readAudioS3(bucket: string, key: string, startSec: number, durSec: number): string {
  // Cache the full S3 file locally, then seek within it.
  // S3 doesn't support partial FLAC reads reliably (byte-range chunks
  // aren't valid FLAC), so we download once and cache.
  return [
    `import os as _os, hashlib as _hl, boto3 as _b3, soundfile as _sf`,
    ...
    `    _b3.client('s3').download_file(_bkt, _key, _cached)`,
