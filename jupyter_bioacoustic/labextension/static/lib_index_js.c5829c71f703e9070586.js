...
class BioacousticWidget extends widgets_1.Widget {
    constructor(tracker) {
        ...
        this._displayCols = [];
        this._dataCols = [];
        this._captureLabel = ''; // empty = hidden
        this._captureDir = '';
    ...
    _buildUI() {
        ...
        refreshBtn.style.cssText = (0, styles_1.btnStyle)() + `font-size:14px;padding:2px 7px;display:none;`;
        ...
        this._canvasContainer.style.cssText =
            `flex:1;position:relative;min-height:80px;background:${styles_1.COLORS.bgCrust};overflow:hidden;cursor:crosshair;`;
        ...
        // ── Assemble widget ──────────────────────────────────────────
        this.node.append(header, filterBar, tableWrap, pagBar, this._infoCard, playerCtrls, this._canvasContainer, playBar, this._audio, this._formSection);
    }
    // ─── Lumino lifecycle ────────────────────────────────────────
    onAfterAttach(msg) {
    ...
    async _init() {
        ...
        let raw;
        try {
            raw = await this._execPython(`import json as _j\n` +
                `print(_j.dumps({\n` +
                `  'data': _BA_DATA,\n` +
        ...
        this._dataCols = JSON.parse(cfg.data_cols);
        this._formConfig = JSON.parse(cfg.form_config);
        this._captureLabel = (_b = cfg.capture) !== null && _b !== void 0 ? _b : '';
        this._captureDir = (_c = cfg.capture_dir) !== null && _c !== void 0 ? _c : '';
        ...
        try {
            this._rows = JSON.parse(cfg.data);
        }
        catch (_e) {
    ...
    _updateFormFromRow(row) {
        ...
        if (this._annotConfig) {
            ...
            if (ac.minFreq)
                this._setAnnotValue('minFreq', null);
            if (ac.maxFreq)
                this._setAnnotValue('maxFreq', null);
        }
    ...
    _showReviewedResult(row) {
        ...
        prevBtn.style.cssText = (0, styles_1.btnStyle)() + `font-size:12px;`;
        prevBtn.disabled = this._selectedIdx === 0;
        prevBtn.addEventListener('click', () => this._onPrev());
        const nextBtn = document.createElement('button');
        nextBtn.textContent = 'Next ▶';
        nextBtn.style.cssText = (0, styles_1.btnStyle)() + `font-size:12px;`;
        ...
        deleteBtn.style.cssText =
            (0, styles_1.btnStyle)() + `font-size:12px;color:${styles_1.COLORS.red};`;
    ...
    async _onDeleteReview(row) {
        ...
        try {
            ...
            this._setStatus(`❌ Delete failed: ${String((_c = e.message) !== null && _c !== void 0 ? _c : e)}`, true);
            return;
        }
    ...
    _selectRow(filteredIdx, autoUpdateInputs = true) {
        ...
        const items = [];
        items.push(mkChip(`${this._fmtTime(row.start_time)} – ${this._fmtTime(row.end_time)}`, styles_1.COLORS.textSubtle));
    ...
    async _loadAudio() {
        ...
        try {
            ...
            this._setStatus(`❌ Image decode: ${String((_b = e.message) !== null && _b !== void 0 ? _b : e)}`, true);
            return;
        }
        ...
        this._setStatus(`✓ ${fname}  ${this._fmtTime(loadStart)}–${this._fmtTime(loadStart + result.duration)}`);
    }
    _buildPythonCode(path, startSec, durSec) {
        const esc = (s) => s.replace(/\\/g, '\\\\').replace(/'/g, "\\'");
        let readLines;
    ...
    _onCanvasMouseDown(e) {
        ...
        if (!this._annotConfig || !this._specBitmap || this._segDuration === 0)
            return;
        ...
        if (tool === 'time_select') {
            ...
            const st = ((_a = ac.startTime) === null || _a === void 0 ? void 0 : _a.col) ? this._formValues[ac.startTime.col] : null;
            const et = ((_b = ac.endTime) === null || _b === void 0 ? void 0 : _b.col) ? this._formValues[ac.endTime.col] : null;
            ...
            const st = ((_c = ac.startTime) === null || _c === void 0 ? void 0 : _c.col) ? this._formValues[ac.startTime.col] : null;
            const et = ((_d = ac.endTime) === null || _d === void 0 ? void 0 : _d.col) ? this._formValues[ac.endTime.col] : null;
            const flo = ((_e = ac.minFreq) === null || _e === void 0 ? void 0 : _e.col) ? this._formValues[ac.minFreq.col] : null;
            const fhi = ((_f = ac.maxFreq) === null || _f === void 0 ? void 0 : _f.col) ? this._formValues[ac.maxFreq.col] : null;
    ...
    _onCanvasMouseMove(e) {
        ...
        if (tgt === 'start') {
            ...
            if (startVal != null)
                this._setAnnotValue('endTime', Math.max(t, startVal));
        }
            ...
            if (loVal != null)
                this._setAnnotValue('maxFreq', Math.max(f, loVal));
        }
            ...
            if (hiVal != null)
                this._setAnnotValue('minFreq', Math.min(f, hiVal));
        }
        this._renderFrame();
        this._updateAnnotDisplay();
    }
    ...
    _updateAnnotCursor(cx, cy) {
        ...
        if (tool === 'time_select') {
            ...
            const st = ((_b = ac.startTime) === null || _b === void 0 ? void 0 : _b.col) ? this._formValues[ac.startTime.col] : null;
            const et = ((_c = ac.endTime) === null || _c === void 0 ? void 0 : _c.col) ? this._formValues[ac.endTime.col] : null;
            if ((st != null && Math.abs(cx - this._timeToX(st)) <= GRAB) ||
            ...
            const st = ((_d = ac.startTime) === null || _d === void 0 ? void 0 : _d.col) ? this._formValues[ac.startTime.col] : null;
            const et = ((_e = ac.endTime) === null || _e === void 0 ? void 0 : _e.col) ? this._formValues[ac.endTime.col] : null;
            const flo = ((_f = ac.minFreq) === null || _f === void 0 ? void 0 : _f.col) ? this._formValues[ac.minFreq.col] : null;
            const fhi = ((_g = ac.maxFreq) === null || _g === void 0 ? void 0 : _g.col) ? this._formValues[ac.maxFreq.col] : null;
            if (st != null && et != null && flo != null && fhi != null) {
                const sx = this._timeToX(st), ex = this._timeToX(et);
                const yhi = this._freqToY(fhi), ylo = this._freqToY(flo);
                const inY = cy >= yhi - GRAB && cy <= ylo + GRAB;
    ...
    _updateAnnotDisplay() {
        ...
        const et = ((_b = ac.endTime) === null || _b === void 0 ? void 0 : _b.col) ? this._formValues[ac.endTime.col] : null;
        if (st != null)
            parts.push(this._fmtTime(st));
        if (et != null)
            parts.push(`– ${this._fmtTime(et)}`);
        const flo = ((_c = ac.minFreq) === null || _c === void 0 ? void 0 : _c.col) ? this._formValues[ac.minFreq.col] : null;
        const fhi = ((_d = ac.maxFreq) === null || _d === void 0 ? void 0 : _d.col) ? this._formValues[ac.maxFreq.col] : null;
    ...
    async _onVerify() {
        ...
        try {
            ...
            this._setStatus(`❌ Write failed: ${String((_a = e.message) !== null && _a !== void 0 ? _a : e)}`, true);
            return;
        }
        this._sessionCount++;
        ...
        // Track as reviewed
        if (!this._duplicateEntries) {
            this._reviewedMap.set(row.id, Object.assign({}, values));
        }
        this._updateProgress();
    ...
    async _onCapture() {
        ...
        const esc = (s) => s.replace(/\\/g, '\\\\').replace(/'/g, "\\'");
        try {
            await this._execPython(`import base64 as _b64, os as _os\n` +
                `_p = '${esc(filename)}'\n` +
...
//# sourceMappingURL=lib_index_js.c5829c71f703e9070586.js.map