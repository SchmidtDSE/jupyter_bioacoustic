...
class FormSection extends CollapsibleSection_1.CollapsibleSection {
    ...
    _buildInlineItems(container, cfg) {
    ...
    _rebuildItemsUI(_container, _mode, _cfg) {
    }
    _moveElement(card, direction) {
        const idx = this._elements.findIndex(e => e.el === card);
        if (idx < 0)
            return;
        const newIdx = idx + direction;
        if (newIdx < 0 || newIdx >= this._elements.length)
            return;
        const [el] = this._elements.splice(idx, 1);
        this._elements.splice(newIdx, 0, el);
        this._listEl.innerHTML = '';
        for (const e of this._elements)
            this._listEl.appendChild(e.el);
        this._emitChanged();
    }
    _removeElement(card) {
        const idx = this._elements.findIndex(e => e.el === card);
    ...
    _rebuildDynFormsUI() {
        ...
        for (const [name, elements] of Object.entries(this._dynamicForms)) {
            ...
            lbl.style.cssText = `font-size:12px;font-weight:700;color:${styles_1.COLORS.mauve};flex:1;`;
...
function buildSpectrogram(spectType, resolutionW, resolutionH) {
    ...
    return [py_chunks_1.buildSpectrogram, filterBlock, resLine, hLine, py_chunks_1.spectrogramRender].join('\n');
}
exports.buildSpectrogram = buildSpectrogram;
/** Python code for a custom visualization callable. */
function customVizCode(vizIndex, resolutionW, resolutionH) {
    return [
        `import numpy as _np, io as _io, base64 as _b64, json as _j`,
...
function spectrogramPipeline(path, startSec, durSec, vizType, builtinKey, vizIndex, resolutionW, resolutionH) {
    ...
    if (vizType === 'custom' && vizIndex != null) {
        return readCode + '\n' + customVizCode(vizIndex, resolutionW !== null && resolutionW !== void 0 ? resolutionW : 2000, resolutionH);
    }
